...
fn thirteen() {
    ...
    // regular functions to functions too! Functions coerce to the type fn, with a
    // lower case ‘f' not to be confused with the Fn closure trait. fn is called a
    // function pointer. The syntax for specifying that a parameter is a function
    // pointer is similar to that of closures:
    ...
    // and FnOnce), so we can always pass a function pointer as an argument when
    // calling a function that expects a closure. Prefer to write functions
    // using a generic type and one of the closure traits, so that your
    // functions can accept either functions or closures. An example of a case
    // where you'd only want to accept fn is when interfacing with external code
    // that doesn't have closures: C functions can accept functions as
    // arguments, but C doesn't have closures.

    // For example, if we wanted to use the map function to turn a vector of
    // numbers into a vector of strings, we could use a closure:
    ...
    // about in the "Advanced Traits" section because there are multiple
    ...
    // Returning Closures
    // Because closures are represented by traits, returning closures is a
    // little tricky; we can't do it directly. In most cases where we may want
    // to return a trait, we can instead use the concrete type that implements
    // the trait of what we're returning as the return value of the function. We
    // can't do that with closures, though. They don't have a concrete type
    // that's returnable; we're not allowed to use the function pointer fn as a
    ...
    // The Sized trait again! Rust doesn't know how much space it'll need to
