...
The algorithm maintains index i as it scans the array using another
index j such that the elements lo through i (inclusive) are less than or
equal to the pivot, and the elements i+1 through j-1 (inclusive) are
greater than the pivot.

#+begin_example
quicksort(A: Vec<i32>, low: u32, high: u32) {
    if low < high {
        let pivot_location = partition(A, low, high);
        quicksort(A, low, pivot_location)
        quicksort(A, pivot_location + 1, high)
    }
}

partition(A: Vec<i32>, low: u32, high: u32){
     let pivot = A[high];
     let i = low;
...
func merge(left, right []int) (result []int) {
    ...
    // fill with the left remaining if any
    for j := 0; j < len(left); j++ {
        result[i] = left[j]
        i++
    }
    // fill with the right remaining if any
...
https://i1.wp.com/www.techiedelight.com/wp-content/uploads/Permutations.png?w=776&ssl=1
https://www.techiedelight.com/find-permutations-given-string/

using [[https://en.wikipedia.org/wiki/Backtracking][backtracking]].
