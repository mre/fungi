...
(&Get, "/") => { let time_range = match request.query() { Some(query)
=> parse_query(query), None => Ok(TimeRange { before: None, after:
None, }), }; let response = match time_range { Ok(time_range) =>
make_get_response(query_db(time_range)), Err(error) =>
make_error_response(&error), }; Box::new(response) } By calling
request.query(), we get an Option<&str>, since a URI may not have a
query string at all. If it is present, we call parse_query, which
...
case, parse_query will return an error message, which we can forward to
the make_error_response function we wrote earlier. Otherwise, we can
go on to call query_db, which will fetch the messages for us, and
make_get_response, which creates an appropriate Response object to
...
fn parse_query(query: &str) -> Result<TimeRange, String> { let args =
...
fn make_get_response( messages: Option<Vec>, ) ->
FutureResult<hyper::Response, hyper::Error> { let response = match
messages { Some(messages) => { let body = render_page(messages);
Response::new() .with_header(ContentLength(body.len() as u64))
.with_body(body) } None =>
...
EXTRACT('epoch' FROM CURRENT_TIMESTAMP) ) Each row in our table stores
...
$ export DATABASE_URL=postgres://:@localhost $ diesel print-schema |
...
#[macro_use] extern crate serde_derive; #[macro_use] extern crate
diesel;

mod schema; mod models; At this point, we are ready to fill in the
functions write_to_db and query_db that we left out earlier.
...
fn write_to_db( new_message: NewMessage, db_connection:
&PgConnection, ) -> FutureResult<i64, hyper::Error> { use
schema::messages; let timestamp = diesel::insert_into(messages::table)
.values(&new_message) .returning(messages::timestamp)
...
return at the same (if any) and call get_result, which will actually
...
(2) we are using a magical db_connection parameter that wasn't there
...
#[derive(Insertable, Debug)] #[table_name = "messages"] pub struct
...
fn call(&self, request: Request) -> Self::Future { let db_connection =
match connect_to_db() { Some(connection) => connection, None => {
...
.and_then(parse_form) .and_then(move |new_message|
write_to_db(new_message, &db_connection))
.then(make_post_response); Box::new(future) } (&Get, "/") => { let
time_range = match request.query() { Some(query) =>
parse_query(query), None => Ok(TimeRange { before: None, after: None,
}), }; let response = match time_range { Ok(time_range) =>
make_get_response(query_db(time_range, &db_connection)), Err(error)
=> make_error_response(&error), }; Box::new(response) } With this
